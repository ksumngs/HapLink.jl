...
"""
    rmdim(A::AbstractArray, dim::Integer)

Reduces the occurence matrix `A` by removing the dimension `dim` and distributing the
occurences to their appropriate indices in the new matrix.

# Example
```jldoctest
julia> mat = reshape([17 2 0 3 5 11 7 19], 2, 2, 2);
...
```
"""
function rmdim(A::AbstractArray, dim::Integer)
    B = similar(A, size(A)[1:(end - 1)])
    fill!(B, 0)
    ...
    for I in CartesianIndices(B)
        for J in CartesianIndices(A)
            ...
            i = CartesianIndex(vcat(pre_j..., post_j...)...)
