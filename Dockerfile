...
RUN --mount=type=secret,id=SSHKEY \
  ...
  cd .. && \
  /tmp/julia/julia-${JULIA_VERSION}/bin/julia -e 'using PackageCompiler; create_app("HapLink.jl", "build", executables=["haplink" => "haplink", "make-haplotype-fastas" => "make_haplotype_fastas"], cpu_target="x86-64")' && \
